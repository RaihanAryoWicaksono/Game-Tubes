...
@export var spanwer_ref: PackedScene
@export var running_enemy_ref: PackedScene
signal new_wave
...
func spawn_new_wave():
	...
	var new_wave = waves[randi()%waves.size()].instantiate()
	
	add_child(new_wave)
	
	for pos in new_wave.get_children():
		var new_spawner = spanwer_ref.instantiate()
		
		add_child(new_spawner)
		
		new_spawner.enemy_to_spawn = running_enemy_ref.instantiate()
		new_spawner.set_position(pos.get_position())
		new_spawner.connect("creature_spawned", Callable(self, "add_enemy"))
...
func add_enemy(enemy, pos):
	...
	enemy.connect("dead", Callable(self, "check_enemies"))
